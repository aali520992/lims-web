  ...
  <d2-container type="card">
    ...
    >
      ...
      <el-form-item label="维修人员:">
        <el-col :span="25">
          <el-input v-model="from.principal"></el-input>
        </el-col>
      </el-form-item>
      <el-form-item label="是否修复" prop="isRepair">
        <el-col :span="25">
          <div>
            <el-select v-model="from.isRepair" placeholder="请选择状态">
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="维修时间">
              <span>{{ props.row.maintainData }}</span>
            </el-form-item>
            <el-form-item label="维修人员">
...
export default {
    ...
    return {
      ...
      table: {
        columns: [
          ...
          { label: '维修人员', prop: 'principal' },
          { label: '维修费用', prop: 'cost' },
          {
            label: '维修时间',
            prop: 'maintainData',
            sortable: 'sortable',
          },
          { label: '是否修复', prop: 'repair' },
        ],
        data: [
          {
            creatorId: '',
  ...
  methods: {
    ...
    getEquipmentRecordList(map = {}) {
      this.$request
        .post('/equipmentRecord/list', map)
        .then((response) => {
          if ((response.code = 200)) {
            this.table.data = response.data.data.records
            this.selectform.pagination.total = response.data.data.total
            for (var i = 0; i < this.table.data.length; i++) {
              if (this.table.data[i].isRepair == 0) {
                this.table.data[i].repair = '未修复'
              } else {
                this.table.data[i].repair = '已修复'
              }
            }
          } else {
            this.$message.error('获取数据失败')
    ...
    // 跳转到新增页面
    insert() {
      this.$router.push('/demo/lab/maintain/add')
    },
    // 导出 Excel
    exportExcel() {
      this.$export
