...
export default {
  data() {
    return {
      ...
      table: {
        columns: [
          { type: 'selection' },
          { label: '学号', prop: 'jobNum' },
          { label: '所属班级', prop: 'roomName' },
  ...
  methods: {
    ...
    // 跳转到新增页面
    insert() {
      this.$router.push('/demo/people/student/add')
    },
    // 导出 Excel
    exportExcel() {
      this.$export
    ...
    onSearch() {
      ...
      } else if (this.from.roomId != '') {
        this.selectdata.field = 'room_id'
        this.selectdata.value = this.from.roomId
        this.selectform.probes.push(this.selectdata)
    ...
    // 学院选择触发
    handleSelect(item) {
      this.from.roomId = item.id
      this.from.roomName = item.value
    },
  },
}
</script>
