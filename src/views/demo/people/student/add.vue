<template>
  <d2-container>
    <template slot="header">添加学生</template>
    <el-form
      :model="ruleForm"
      status-icon
      :rules="rules"
    ...
    >
      ...
      <el-form-item label="所属班级" prop="roomName">
        <el-col :span="6">
          <el-autocomplete
            class="inline-input"
            v-model="ruleForm.roomName"
            :fetch-suggestions="searchRoom"
            placeholder="请输入内容"
            @select="handleRoom"
          ></el-autocomplete>
        </el-col>
      ...
      <el-form-item label="学号" prop="jobNum">
        <el-col :span="6">
...
export default {
  ...
  methods: {
    ...
    //获取班级
    searchRoom(queryString, cb) {
      var restaurants = this.roomList
      var results = queryString
        ? restaurants.filter(this.createFilter(queryString))
        : restaurants
      // 调用 callback 返回建议列表的数据
    ...
    // 获取教师列表
    getRoomList(professionId) {
      var url = '/room/roomList/' + professionId
      this.$request
        .post(url)
        .then((response) => {
          if ((response.code = 200)) {
          ...
          this.$request
            ...
            .then((response) => {
              if ((response.code = 200)) {
                ...
                this.$router.push('/demo/people/student/list')
              } else {
    ...
    handleRoom(item) {
      ...
      this.ruleForm.RoomName = item.value
    },
  },
}
</script>
