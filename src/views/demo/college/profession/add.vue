<template>
  <d2-container>
    <template slot="header">添加专业</template>
    <el-form
      :model="ruleForm"
      status-icon
      :rules="rules"
    ...
    >
     <el-form-item label="所属学院" prop="collegeName">
      ...
      <el-form-item label="专业简介" prop="synopsis">
        <el-col :span="6">
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$request
            .post('/profession/insert', this.ruleForm)
            .then((response) => {
              if ((response.code = 200)) {
                this.$message.success('添加成功')
                this.$router.push('/demo/college/profession/list')
              } else {
